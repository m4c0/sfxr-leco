...
bool ddkCalcFrame() {
  ...
  if (input->KeyPressed(DIK_SPACE) ||
      input->KeyPressed(DIK_RETURN)) // WIN32 (keyboard input only for
...
void ddkInit() {
  ...
  if (LoadTGA(font, "/usr/share/sfxr/font.tga")) {
    ...
    if (LoadTGA(font, "font.tga")) {
      fprintf(stderr, "Error could not open /usr/share/sfxr/font.tga"
                      " nor font.tga\n");
      exit(1);
    }
  }
  ...
  if (LoadTGA(ld48, "/usr/share/sfxr/ld48.tga")) {
    ...
    if (LoadTGA(ld48, "ld48.tga")) {
      fprintf(stderr, "Error could not open /usr/share/sfxr/ld48.tga"
                      " nor ld48.tga\n");
      exit(1);
    }
  }
